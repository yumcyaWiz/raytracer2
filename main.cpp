...
int main(int argc, char** argv) {
    ...
    Camera *cam = new EquidistantFisheyeCamera(camPos, camForward, 1.5f, 1.0f);
    ...
    objs->add(new Sphere(&t2, &t2_inv, new Glass(1.4f), new ImageTexture("earth2.jpg"), 1.0f, -1.0f, 1.0f, 2*M_PI));
    Transform t3 = translate(Vec3(3, 0, 0));
    Transform t3_inv = inverse(t3);
    objs->add(new Sphere(&t3, &t3_inv, new Glass(1.4f), new Mono(RGB(1.0f)), 1.0f, -1.0f, 1.0f, 2*M_PI));
    
    Transform t4 = translate(Vec3(-3, 0, 0));
    Transform t4_inv = inverse(t4);
    objs->add(new Sphere(&t4, &t4_inv, new Mirror(1.0f), new Mono(RGB(1.0f)), 1.0f, -1.0f, 1.0f, 2*M_PI));
    /*
